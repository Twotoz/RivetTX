...
| OpenPocket VRX/video OSD | non-blocking 6×8 frequency controller and scan state machine, RSSI/video-loss state, fixed 30×16 character compositor; this is the OpenPocket menu/status presentation instead of OLED | native tests only; no physical VRX or AT7456E-class driver until the OpenPocket hardware profile is frozen |
